...
export default function CommandCenter() {
  ...
  // --- PERSISTED MEMORY ENGINE INTEGRATION ---
  const [persistentMemories, setPersistentMemories] = useState<any[]>([]);
  useEffect(() => {
    fetch("/api/empire/memory")
      .then(r => r.json())
      .then(data => {
        if (data.success) {
          setPersistentMemories(data.memories);
        }
      })
      .catch(err => console.error("Error fetching memories in command center:", err));
  }, []);
    ...
    <div className="bg-zinc-900 border border-zinc-850 rounded-xl p-5 space-y-6 animate-fade-in font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-zinc-950/40 border border-zinc-850/80 rounded-xl p-4 flex flex-col justify-between space-y-4">
          ...
          <div className="bg-zinc-950/80 border border-zinc-850 rounded-lg p-2.5 flex items-center justify-between gap-3 text-[10px] font-mono">
            <div className="flex items-center gap-1.5 text-indigo-400">
              <Database className="w-3.5 h-3.5 text-indigo-400 animate-pulse" />
              <span className="font-bold">UNIFIED MEMORY:</span>
              <span className="bg-indigo-950 text-indigo-300 px-1.5 py-0.5 rounded border border-indigo-900/40 font-extrabold">{persistentMemories.length} KEYS</span>
            </div>
            <div className="flex gap-1.5 text-zinc-500 overflow-hidden text-[9px]">
              {persistentMemories.slice(0, 2).map((m: any) => (
                <span key={m.id} className="bg-zinc-900 px-1 py-0.5 rounded truncate max-w-[120px] text-zinc-400">
                  #{m.key}
                </span>
              ))}
              {persistentMemories.length > 2 && <span>+{persistentMemories.length - 2} more</span>}
            </div>
          </div>
        </div>
